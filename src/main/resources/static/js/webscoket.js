...
window.onload = function() {
    ...
    if (localStorage.getItem("userId") === null) { //here we use userId
...
//    }
    ...
    fetchAll();
    connectToChat(localStorage.getItem("userId")); //here we use userId

  };

function fetchAll() {

    var userId = localStorage.getItem("userId"); //here we use userId
    ...
    $.get(url + "/fetchAllUsers/"+userId, function (response) {
        ...
        for (let i = 0; i < users.length; i++) {
            console.log(users[i]['first_name'])
                ...
                usersTemplateHTML = usersTemplateHTML + '<li class="active" id="child_message" onclick="formMessageLauch('+users[i]['id']+',\''+users[i]['first_name']+'\',\'user\')" data-userid="'+users[i]['id']+'" data-type="user">'+
                ...
                '<span>'+users[i]['first_name']+'</span>'+
                '<p>'+users[i]['first_name']+' is online</p>'+
                '</div>'+
                '</div>'+
                '</li>';
        }
...
function logout(userName){
    ...
    localStorage.removeItem("userId"); //here we use userId
    //window.location.href = "index.html";
...
//function getID(){
//    localStorage.setItem("userId",[${#request.userPrincipal.principal.id}]);
//    return true;
//
//}
